...
const createFilterWasBornAfter = (year) => {
  return (person) => person.born > year;
};
const compareByBorn = (a, b) => {
  return a.born - b.born;
};
const createComparatorByField = (field) => {
  return (a, b) => {
    if (a[field] > b[field]) {
      return 1;
...
const ComparatorByName = createComparatorByField("name");
const whoDiedLater = createComparatorByField("died");
const wasBornAfter1700 = createFilterWasBornAfter(1700);
const wasBornAfter1825 = createFilterWasBornAfter(1825);

console.log(
  people
    .filter(wasBornAfter1825)
    .sort(ComparatorByName)
    .map((person) => `${person.name} -- ${person.born}`)
);
...
test(
  +calculateMenAverageAge(people, 18).toFixed(2),
  56.5,
  "Average men age in 18 century"
);
test(+calculateWomenAverageAge(people).toFixed(2), 54.56, "Average women age");
test(
  +calculateWomenAverageAge(people, true).toFixed(2),
  54.15,
  "Average mothers age"
);
test(
  +calculateAverageAgeDiff(people).toFixed(2),
  31.22,
  "Average age difference"
);
test(
  +calculateAverageAgeDiff(people, true).toFixed(2),
  30.08,
